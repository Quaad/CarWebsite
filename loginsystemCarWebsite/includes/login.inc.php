...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($mailuid) || empty($password)) {
    ...
    // gettubg the password from the user in the database that has the same username as what the user typed in, then de-hash it and check if it matches the password the user typed into the login form.

    // connect to the database using prepared statements which work by us sending SQL to the database first, then fill in the placeholders by sending the users data.
